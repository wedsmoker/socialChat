...
router.get('/analytics', requireAdmin, async (req, res) => {
  try {
    ...
    const recentVisitors = await query(
      `SELECT ip_address, path, method, visited_at, user_agent
       FROM visitor_logs
       WHERE visited_at >= NOW() - INTERVAL '14 days'
    ...
    const visitsPerDay = await query(
      `SELECT DATE(visited_at) as date, COUNT(*) as visits, COUNT(DISTINCT ip_address) as unique_ips
       FROM visitor_logs
       WHERE visited_at >= NOW() - INTERVAL '14 days'
    ...
    res.json({
      ...
      visitsPerDay: visitsPerDay.rows
    });
  } catch (error) {
